 ...
 *   Modifications for Mac OS X by Da-Woon Jung are enclosed in
 *   TARGET_OS_MAC defines.
 *
 */
#include "pch.h"
#include "Emu48.h"
	...
	if (bCpuSlow || bKeySlow)				// emulation slow down
	{
		...
		EnterCriticalSection(&csSlowLock);
		{
			...
			if ((dwCycles = (DWORD) (Chipset.cycles & 0xFFFFFFFF)-dwOldCyc) >= (DWORD) T2CYCLES)
			{
                ...
                if (dwTicks < dwTickRef)
                {
                    static struct mach_timebase_info timebase;
                    if (0 == timebase.denom)
                        mach_timebase_info(&timebase);
                    tv.tv_sec = 0;
                    tv.tv_usec = dwTickRef - (dwTicks * timebase.numer/timebase.denom * 0.001);
                    select(0,0,0,0, &tv);
                }
#else
				do
